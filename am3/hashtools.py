...
def hash_directory_recursive(full_path_to_dir, root_str=".", exclude_re=None, include_re=None, basename=True):
    if root_str == "":
        separator = ""
    else:
        separator = os.sep
...
def generate(folder):
    ...
    output_list = []
    hash_list = hash_directory_recursive(folder)
    for item in hash_list:
        output_list.append(output_template.format(*item))

    return output_list

def generate_single_file(filename):
    output_template = """File: {}
    Size:     {}
    CRC32:    {}
    MD5:      {}
    ...
    path_string = f"{os.path.basename(filename)}"
    size = os.path.getsize(filename)
    return output_template.format(path_string, size, *hashes)